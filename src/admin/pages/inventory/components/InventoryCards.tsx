...
const InventoryCards = ({ productsData, materialsData } : InventoryCardsType ) => {
  
  const lowStockCount = productsData?.filter((p) => (p.stocks || 0) < 10)?.length;
  const outOfStockCount = productsData?.filter((p) => (p.stocks || 0) === 0)?.length;
