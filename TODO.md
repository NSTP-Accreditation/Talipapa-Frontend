...
**Issue:**  
Heavy components like `Dashboard.tsx`, `Records.tsx`, and `Inventory.tsx` re-render unnecessarily when parent components update.
...
- `src/admin/pages/records/ResidentRecords.tsx`
...
**Testing:** Verify no regressions with React DevTools Profiler
...
**Priority:** 🟡 MEDIUM  
**Effort:** 1-2 days  
**Location:** `src/admin/hooks/useAuthFetch.ts`

**Issue:**  
Multiple components making identical API calls simultaneously (e.g., fetching user data).
...
**Usage in useAuthFetch:**

```typescript
const cacheKey = `${method || 'GET'}:${finalUrl}`;
return requestCache.dedupe(cacheKey, () => fetch(...));
```
...
**Issue:**  
AuthContext causes all children to re-render when user object updates, even if most don't need the user data.
...
// Usage
const { login, logout } = useAuthActions(); // Doesn't cause re-renders
...
**Priority:** 🟡 MEDIUM  
**Effort:** 2-3 days  
**Location:** Records and Inventory pages

**Issue:**  
Large lists (1000+ records) cause performance issues when rendering all at once.
...
function RecordsList({ records }) {
  const Row = ({ index, style }) => (
    <div style={style}>{records[index].name}</div>
  );
...
**Issue:**  
Large bundle size (potentially) due to no chunk splitting configuration.
...
**Analysis:** Run `npm run build -- --mode analyze` to visualize bundle
